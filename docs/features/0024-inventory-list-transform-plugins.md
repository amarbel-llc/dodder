...
the VM pool's existing sandboxing (no file I/O, no network, `require()`
restricted to the `der`/`dodder`/`zit` module aliases) — a real security
property worth keeping for a plugin that gets raw blob-write power.
