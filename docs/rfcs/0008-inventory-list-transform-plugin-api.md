...
repo. `clone -script` adds the transform to clone's existing genesis+pull, and
works over **every transport**. Direct (local-path) and legacy HTTP buffer
through the `repo.Repo` surface (`MakeInventoryList` for the object set +
`GetBlobStore` for the blob pre-copy); the drtp/websocket transport buffers
through `client.FetchToBuffer`'s staging mode — `receiveClosure` decodes the
objects frame into a buffer instead of importing inline, and since its blobs
already stream into the clone's store, no pre-copy is needed there
(dodder#396). All reuse the same `-script`/`-script-digest` and validation
