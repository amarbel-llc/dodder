...
func DecodeV3Into(data *V3, doc *document.Document, container *cst.Node, consumed map[string]bool, keyPrefix string) error {
	for _, _kv := range container.Children {
		...
		case "ignore":
			if v, ok := cst.ExtractStringSlice(_kv); ok {
				data.Ignore = v
				consumed[keyPrefix+"ignore"] = true
			}
		}
	}
