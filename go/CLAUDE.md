...
- **Update flake input**: `just update-flake-input <input>` (e.g.,
  ...
  For the seven `flake-input-go_mod`-bridged amarbel deps (madder, hyphence,
  piggy, tap, tommy, purse-first/dewey, purse-first/go-mcp), **flake.lock is
  the source of truth**:
...
- **Format**: `just codemod-go-fmt` (runs `nix fmt`, the conformist formatter:
  goimports + gofumpt for Go, plus nixfmt / shfmt / stylua / rustfmt / tommy;
  config is Nix-generated from the repo-root `conformist.nix` — see
  conformist-nix(7))
...
the `complete_subcmd` test in `zz-tests_bats/complete.bats`.
