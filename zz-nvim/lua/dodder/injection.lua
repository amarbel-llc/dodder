...
-- Fast, flicker-free fallback keyed on the dodder *type string* captured from
-- the "! type" metadata line (e.g. `md`, `task`, `toml-type-v2`). The async
-- resolver corrects anything not covered here. Most objects are md/task
-- (markdown) or a *config*/type blob (toml).
local type_to_lang = {
  md = 'markdown',
  markdown = 'markdown',
  task = 'markdown',
...
function M.static_lang(type_string)
  ...
  -- Versioned config/type blobs (toml-...-vN, ...-config-...) are TOML.
  if type_string:match('^toml%-') or type_string:match('config') then
    return 'toml'
  end
  return type_to_lang[type_string] or 'markdown'
end
...
function M.resolve_async(bufnr)
  ...
  local bin = vim.g.dodder_bin or vim.env.BIN_DODDER or 'dodder'
